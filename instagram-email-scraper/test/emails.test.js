...
describe('extractEmailsFromText', () => {
    it('pulls addresses out of a plain string', () => {
        assert.deepEqual(extractEmailsFromText('Piši na office@firma.rs ili prodaja@firma.rs!'), [
            'office@firma.rs',
            'prodaja@firma.rs',
        ]);
    });

    it('de-obfuscates by default and can be told not to', () => {
        assert.deepEqual(extractEmailsFromText('booking [at] firma [dot] rs'), ['booking@firma.rs']);
        assert.deepEqual(extractEmailsFromText('booking [at] firma [dot] rs', { deobfuscate: false }), []);
    });

    it('de-duplicates, sorts and applies the blocklist', () => {
        assert.deepEqual(
            extractEmailsFromText('a@keep.rs A@KEEP.RS b@drop.rs', { blockedDomains: ['drop.rs'] }),
            ['a@keep.rs'],
        );
    });
...
describe('extractEmailsFromPage', () => {
    ...
    it('does not glue adjacent elements into a bogus local part', () => {
        const html = '<body><h1>Firma</h1><p>office@firma.rs</p></body>';
        assert.deepEqual(scrape(html, { scanRawHtml: false }), ['office@firma.rs']);
    });

    it('keeps a label and its address in separate cells apart', () => {
        const html = '<body><table><tr><td>E-mail</td><td>prodaja@firma.rs</td></tr></table></body>';
        assert.deepEqual(scrape(html, { scanRawHtml: false }), ['prodaja@firma.rs']);
    });
