...
import { instagramHandleFromUrl, isLinkHub, scrapeWebsiteForContacts, scrapeWebsiteForEmails } from '../src/website.js';
...
describe('instagramHandleFromUrl', () => {
    it('pulls the handle out of a profile link', () => {
        assert.equal(instagramHandleFromUrl('https://www.instagram.com/salon_ana/'), 'salon_ana');
        assert.equal(instagramHandleFromUrl('https://instagram.com/Salon.Ana?igshid=x'), 'salon.ana');
    });

    it('ignores posts, reels and other features', () => {
        assert.equal(instagramHandleFromUrl('https://www.instagram.com/p/Cabc123/'), null);
        assert.equal(instagramHandleFromUrl('https://www.instagram.com/explore/tags/nis/'), null);
        assert.equal(instagramHandleFromUrl('https://www.instagram.com/'), null);
    });

    it('ignores lookalike hosts and invalid handles', () => {
        assert.equal(instagramHandleFromUrl('https://instagram.com.evil.rs/salon/'), null);
        assert.equal(instagramHandleFromUrl('https://www.instagram.com/ime%20sa%20razmakom/'), null);
        assert.equal(instagramHandleFromUrl('nije url'), null);
    });
});

describe('scrapeWebsiteForContacts', () => {
    it('collects Instagram handles alongside e-mails', async () => {
        await withServer(
            {
                '/': '<html><body><p>office@salonana.rs</p>'
                    + '<a href="https://www.instagram.com/salon_ana/">IG</a>'
                    + '<a href="https://www.instagram.com/p/Cabc/">objava</a>'
                    + '<a href="https://facebook.com/salonana">FB</a></body></html>',
            },
            async (base) => {
                const { emails, instagramHandles } = await scrapeWebsiteForContacts(base);

                assert.deepEqual(emails, ['office@salonana.rs']);
                assert.deepEqual(instagramHandles, ['salon_ana'], 'post links must not become handles');
            },
        );
    });

    it('returns empty arrays for a site with no contacts at all', async () => {
        await withServer(
            ...
            async (base) => {
                ...
                assert.deepEqual(instagramHandles, []);
            },
        );
    });
});
